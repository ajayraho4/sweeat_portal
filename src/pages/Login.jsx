		...
		async function getData(){
			try {
				...
				if(docSnap.docs.length===1){
					...
					handleClickOpen("No such account found. Recheck entered credentials.")
					setLoading(false)
				}
			} catch (err) {
				...
				<div className='flex justify-center rounded-lg border py-8 px-8 w-3/6 mt-5 drop-shadow-md bg-white'>
					<form className='flex flex-col justify-center items-center space-y-3 w-full' onSubmit={handleSubmit}>
						<div className='inputBox'>
							...
							<input placeholder='Admin Email' required onChange={(e)=>setEmail(e.target.value)} type="email" />
						</div>	
						<div className='inputBox'>
							<div className='px-2'>
							...
							<input placeholder='Password' required onChange={(e)=>setPasssword(e.target.value)} type="password" />
						</div><br />
						<button type="submit" disabled={loading} onClick={handleSubmit} className='button button-purple w-32'>
							{loading ? 
